...
dict_port_service = {20: 'FTP-DATA', 21: 'FTP', 22: 'SSH', 23: 'TELNET', 25: 'SMTP',
                     53: 'DNS', 69: 'TFTP', 80: 'HTTP', 110: 'POP3', 161: 'SNMP',
                     443: 'HTTPS', 445: 'MICROSOFT-DS', 8080: 'HTTP'}
...
def get_field(unify_dir):
    files_path = []
    files = os.listdir(unify_dir)
    for file in files:
        files_path.append(os.path.join(unify_dir, file))
    files_path.sort()
    print('unified files path: ', files_path)
    ...
    for file_path in files_path:
        ...
        with open(file_path, 'rb') as f:  # 要以rb方式打开，用r方式打开会报错
            ...
            for ts, raw_data in pkts:
                ...
                trans_dport = trans.dport
                dict_fields['trans_sport'] = trans_sport
                dict_fields['trans_dport'] = trans_dport
                if trans_sport in dict_port_service:
                    dict_fields['comm_way'] = dict_port_service[trans_sport]
                elif trans_dport in dict_port_service:
                    dict_fields['comm_way'] = dict_port_service[trans_dport]
                else:
